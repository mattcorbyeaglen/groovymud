...
class DoorImpl extends ExitImpl implements HeartBeatListener{
	...
	private def doorEvent(obj, boolean open){
		this.open = open
		if(otherDoor == null){
			this.otherDoor = findOtherDoor(arrivalDirection, destination)
			this.otherDoor.setOpen(open)
			this.otherDoor.setOtherDoor(this)
		}
		fireDoorEvent(obj)
	}
	...
	def findOtherDoor(String direction, ObjectLocation location){
		def otherRoom = getObjectAttendant().load(location)
		if(otherRoom != null){
